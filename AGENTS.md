...
- No runtime deps beyond `dom-expressions`, imported only under `src/ui/overlay/` (fallback for browsers without CSS anchor positioning; drop when Firefox ships `position-area`).
