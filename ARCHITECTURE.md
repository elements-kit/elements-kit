...
| `elements-kit/suspense` | `Suspense` — loading boundaries over the §11 async machinery (code splitting = `async` + dynamic import) | experimental |
...
- **Suspense & code splitting** (`elements-kit/suspense`): code splitting is `async(() => import(…))` — the stream awaits the import in order, hydration defers `run()` and keeps server content until the chunk lands. `Suspense` shows its fallback only on the client while direct async children (or `when`) are pending; it stamps the region with ids + a pending-probe so the claim walk keeps server content (no fallback flash) and stays ek-data-aligned past the boundary. One async child per boundary is the supported SSR-hydration shape; props for code-split components use the element-factory recipe (`promise(op.then((C) => () => <C …/>))`).
