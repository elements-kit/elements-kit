...
#### Custom element — `[SLOTS]` for named slots
...
class XCard extends HTMLElement {
  // Named slots — keys flow into JSX as `slot:header` / `slot:footer`.
...
`Props<XCard>` already infers slots from `[SLOTS]` — prefer that when you have a concrete class to point at. `SlotProps<K>` is the **fallback** for cases where you can't derive from an instance: function components that explicitly want `slot:name` syntax, or hand-written prop interfaces.
