...
| Wrap caller-passed props in reactive | [`MaybeReactiveProps<P>`](#maybereactivepropsp) |
| Function-component received props (auto-wrapped) | [`ReactiveProps<P>`](#reactivepropsp) |
| Recover raw `P` from a branded `ReactiveProps<P>` | [`RawProps<R>`](#rawpropsr) |
| Translate any component to its JSX call-site shape | [`ResolveProps<C, P>`](#resolvepropsc-p) |
...
Plus the namespaced extras (`class:`, `style:`, `prop:`, `slot:`, `ref`) are layered on every intrinsic element via `JsxNamespaces` in [src/jsx-runtime/index.ts](https://github.com/wbettayeb/elements-kit/blob/main/src/jsx-runtime/index.ts).
...
`PropsOf` accepts **components only** — a plain prop shape like `PropsOf<{ name: string }>` is a type error. To transform a raw shape, use [`MaybeReactiveProps<P>`](#maybereactivepropsp), which also preserves required keys (`PropsOf` makes every instance field optional).
...
- Empty constructor param (instance-field classes) → `PropsOf<C>`.
