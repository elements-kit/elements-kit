...
- **Compose, don't configure.** Small focused APIs — `signal`, `computed`, `on`, `fromEvent`, `async`. State, server-state, browser hooks, and UI all live in one toolkit.

- **Designed for the AI age.** Code is cheap; maintenance still isn't. Primitives compose into higher-level blocks whose units stay testable in isolation, letting you swap one block at a time, instead of maintaining an overloaded interface. 
...
<CardGrid>
  ...
  <LinkCard title="Philosophy" href="/getting-started/philosophy" description="Four principles behind the API — compose don't configure, close to the platform, predictable and explicit, designed for the AI age." />
</CardGrid>
