...
The getter returns the `Slot` (your `render()` keeps calling `this.header.render()`); the setter takes a `Node` — call `el.header.clear()` to empty it. `PropertiesOf` types the key as `Node`, and inside elements-kit JSX `header={…}` keeps working.
