...
---
// Step 6 — render + unmount + defineElement: scoped mount, typed JSX
import { computed, reactive } from "elements-kit/signals";
import { attributes, ATTRIBUTES as attr } from "elements-kit/attributes";
...
| 6 | `elements-kit/render` + `defineElement` | Scoped mount with a single `unmount` thunk, plus typed JSX via `CustomElementRegistry` augmentation |
...
class ClockElement extends HTMLElement {
  ...
  #template = () => {
    ...
    return <time>{() => this.#time().toLocaleTimeString()}</time> as Element;
  }
