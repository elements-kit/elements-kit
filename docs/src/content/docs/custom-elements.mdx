...
Declare a named slot with the `@slot()` decorator — the property IS the slot:
...
class XCard extends HTMLElement {
  ...
  render() {
    ...
    // or, without elements-kit JSX: root.append(this.header)
  }
}

const el = document.querySelector("x-card")!;
el.header = document.createElement("h1"); // fills the slot
el.header = new Text("plain text");       // any Node
el.header = null;                         // clears it
```

Two rules:

- **Reading places the region.** The getter yields the slot's comment-marker fragment — append it wherever the content should live. Any renderer that can append a fragment works; no elements-kit JSX required. Reads are for *placement*, not inspection (a later read extracts the current content — the re-render semantic).
- **Assigning fills it.** Any `Node`, from any framework or none; `null` clears. Assignments before the region is placed are buffered and flush on mount, so consumers can set content before the element first renders.

`PropertiesOf` types the key by its declared field type (`Node`), and inside elements-kit JSX `header={…}` works as a plain prop — including reactive values (`header={computed(…)}` re-fills on change).
