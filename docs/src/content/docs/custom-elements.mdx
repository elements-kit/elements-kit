...
Augment `CustomElementRegistry` from `"elements-kit/custom-elements"` instead. The JSX runtime reads tag names from that interface to type props and refs.
...
The same pattern applies when registering with `customElements.define` directly — augment `CustomElementRegistry` regardless of which registration call you use.
