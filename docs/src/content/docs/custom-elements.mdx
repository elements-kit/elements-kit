...
To make slot content settable by plain assignment (`el.header = node` — the only slot API other frameworks can reach), pair the `Slot` with a getter/setter:
...
The getter returns the `Slot` (your `render()` keeps calling `this.header.render()`); the setter takes a `Node` — call `el.header.clear()` to empty it. `PropertiesOf` types the key as `Node`, and inside elements-kit JSX both `header={…}` and `slot:header={…}` keep working.
