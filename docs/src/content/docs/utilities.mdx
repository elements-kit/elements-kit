...
| `onMount` | `utilities/on-mount` | `Computed<R \| undefined>` |
...
`target` accepts a static `Element` or a `Signal<Element | null>` / `Computed<Element | null>` — when reactive, the watch swaps with the current value. `fn`'s return value is exposed via the returned `Computed<R | undefined>`, so `onMount` doubles as a "produce a mount-only value" primitive (this is what [`useContext`](#usecontext) builds on). Cleanup for resources allocated inside `fn` goes through `onCleanup`:
...
```ts
function onMount<E extends Element, R>(
