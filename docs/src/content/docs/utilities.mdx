...
| `useContext` | `utilities/context` | `Computed<T \| undefined>` |
| `onMount` | `utilities/on-mount` | `void` |
| `observeRoot` | `utilities/on-mount` | `void` |
...
Run a callback once an element is connected to the DOM, and re-run on every (re)connection. Useful when a JSX `ref` callback fires too early — e.g. resolving `getContext`, measuring layout, attaching observers that need a connected ancestor. Composed-tree aware: shadow-DOM (dis)connect events propagate correctly through nested open shadow roots.
...
function FocusOnMount() {
  const elRef = signal<HTMLInputElement | null>(null);
  onMount(elRef, (el) => el.focus(), { once: true });
  return <input ref={(el) => elRef(el)} />;
}
```

`target` accepts a static `Element` or a `Signal<Element | null>` / `Computed<Element | null>` — when reactive, the watch swaps with the current value. `fn` may **return a disposer** that runs on each disconnect (and on full teardown of the surrounding scope):
...
`fn` runs **outside** any internal effect scope. `onCleanup` calls inside `fn` are no-ops. To bind reactive lifetime to mount, wrap explicitly and return the disposer:

```tsx
import { effectScope, effect } from "elements-kit/signals";
...
By default `fn` re-runs on every reconnection (item moves in `<For>`, portal moves between parents). Pass `{ once: true }` to fire at most once per element.

```ts
function onMount<E extends Element>(
...
Pre-registers a `MutationObserver` on a root so future `onMount` registrations there work without surprises. Two situations call for it:

1. **Portals into a previously-untouched shadow root** — async reattach into a shadow tree no `onMount` has visited would otherwise be missed.
2. **Closed shadow roots** — `node.shadowRoot` returns `null` from outside, so the composed-tree walk can't descend on its own. The owner of the closed shadow opts it in by passing the reference to `observeRoot`.

```ts
import { observeRoot } from "elements-kit/utilities/on-mount";

class Widget extends HTMLElement {
  #shadow = this.attachShadow({ mode: "closed" });
  connectedCallback() {
    observeRoot(this.#shadow);  // opt the closed root in
    // …render into this.#shadow…
  }
}
```

Fire-and-forget: call once during setup, no teardown needed. The observer is shared by every entry on that root and persists for the root's lifetime (one per `Document` or `ShadowRoot`).

```ts
function observeRoot(root: Document | ShadowRoot): void
```

:::caution[Closed shadow roots]
Without `observeRoot`, a closed shadow root's content is unobservable from outside — outer-tree mutations (host append/remove) cannot descend into the closed tree. This is a platform contract, not a library limitation.
:::
...
The consumer must be in the DOM tree at call time. Inside a JSX `ref` callback the element has not yet been inserted, so a synchronous `getContext(el, …)` returns `undefined`. Use [`useContext`](#usecontext) — the JSX-friendly composer that defers the lookup to mount time and auto-flattens reactive values:
...
JSX-friendly composer over [`onMount`](#onmount) + `getContext`. Waits until `target` is connected, walks ancestors for `key`, and **auto-flattens** when the registered value is a `Signal` / `Computed` — the consumer reads the current value with a single call. Returns a `Computed<T | undefined>` that stays `undefined` until the consumer is connected.

Use this from a component body where you'd otherwise reach for `getContext` inside a `ref`. For synchronous lookups inside `connectedCallback` (host already connected) prefer `getContext` directly.
