...
| `createElementRect` | `utilities/element-rect` | `Computed<DOMRect> & Disposable` |
...
| `FormObject` | `utilities/form-object` | `{ toObject, fromObject, clear }` — see [Form object](/utilities/form-object) |
...
Observes the full bounding rect of `target` and returns it as ONE reactive `DOMRect`, so every field a reader sees was measured at the same instant. Size changes come from a `ResizeObserver`; position changes from capture-phase `scroll` and window `resize`, since a `ResizeObserver` stays silent when an element merely moves.

`target` may be a getter, in which case the observer follows it and the returned computed keeps its identity across the swap.

The value is cached, not measured per read — it refreshes when a source fires, so reading never forces a layout.

```ts
import { createElementRect } from "elements-kit/utilities/element-rect";

const rect = createElementRect(document.querySelector("#box")!);

effect(() => console.log(rect().width, rect().height));
```
...
The consumer must be in the DOM tree at call time. Inside a JSX `ref` callback the element has not yet been inserted, so a synchronous `getContext(el, …)` returns `undefined`. Defer the lookup to mount time with [`<dom-lifecycle>`](/components/dom-lifecycle/):
...
For a component body that needs `getContext` after connection, defer with [`<dom-lifecycle>`](/components/dom-lifecycle/) — it's render-inert (zero layout box, no a11y role) and its `onConnect` fires once the consumer is in the tree:
