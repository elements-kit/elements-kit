...
The consumer must be in the DOM tree at call time. Inside a JSX `ref` callback the element has not yet been inserted into its parent, so defer the lookup with `queueMicrotask` — the whole subtree is mounted by the next microtask:
  ...
  return (
    <button
      ref={(el) => {
        ...
        queueMicrotask(() => {
          const theme = getContext<() => "light" | "dark">(el, THEME);
          if (theme) stop = effect(() => { el.dataset.theme = theme(); });
        });
        onCleanup(() => stop?.());
      }}
    >
...
For one-off reads (no reactive binding), call `getContext` from an event handler — the tree is always connected by the time the handler fires:
