...
| `data-color` | accent name — see [accent list](#customization) |
| `data-base-color` | `gray` · `mauve` · `olive` · `sage` · `sand` · `slate` |
...
- `--base-color-1`..`--base-color-12` plus `--base-color-a1`..`--base-color-a12` — the active gray (set by [`base/<scale>.css`](https://github.com/elements-kit/elements-kit/tree/main/src/ui/styles/base) or `data-base-color`).
- `--color-1`..`--color-12` plus `--color-a1`..`--color-a12` — the active accent (set by [`colors/<color>.css`](https://github.com/elements-kit/elements-kit/tree/main/src/ui/styles/colors) or `data-color`).
...
```css
.brand-card {
  --color-material-solid: var(--color-2);
...
Available accents: `amber`, `blue`, `bronze`, `brown`, `crimson`, `cyan`, `gold`, `grass`, `gray`, `green`, `indigo`, `iris`, `jade`, `lime`, `mint`, `orange`, `pink`, `plum`, `purple`, `red`, `ruby`, `sky`, `teal`, `tomato`, `violet`, `yellow`. Plus semantic aliases that point at the active gray or a fixed hue: `base`, `success`, `error`, `warning`, `info`.

Available base (gray) colors (pick one): `gray`, `mauve`, `olive`, `sage`, `sand`, `slate`.
