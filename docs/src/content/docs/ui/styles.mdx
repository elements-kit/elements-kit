...
Scales — `--gray-1`..`--gray-12` plus `--gray-a1`..`--gray-a12` (alpha). Same shape for `--color-*` (the active accent), plus `--black-a*` and `--white-a*` from `colors/black-alpha.css` / `colors/white-alpha.css` (auto-imported by `theme.css`).
