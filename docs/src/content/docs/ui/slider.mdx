...
import SLIDER from "@/playground/files/ui-slider.tsx?raw";
...
<Playground
  provider={{ files: { "/main.tsx": { code: SLIDER, active: true } } }}
  preview={{}}
/>
...
- **`surface`** *(default)* — bordered track, `--base-color-a3` background, 1px `--base-color-a5` border.
- **`soft`** — flat track, `--base-color-a4` background, no border, thumb has tinted shadow.

## Focus

`:focus-visible` adds a 3px `--color-3` ring + 5px `--focus-8` outline around the thumb. The native track outline is suppressed.
