...
`data-variant="material"` turns the group into a floating capsule, as in iOS 26 toolbars. Children sit apart instead of joining (no overlap, no flattened corners) on a `--color-material` background with its backdrop blur and `--shadow-3`.
