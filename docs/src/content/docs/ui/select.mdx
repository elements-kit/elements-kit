...
- **`surface`** *(default)* — `transparent` background, thin `--base-color-a7` inset ring, neutral `--base-color-12` text. Hover deepens the ring to `--base-color-a8`. Surface stays neutral even when an ancestor sets `data-accent`.
- **`soft`** — `--accent-a3` accent-tinted track, `--accent-12` text. Hover bumps the track to `--accent-a4`. Without an accent, `--accent-*` falls back to `--base-color-*` and the select stays gray-tinted.
