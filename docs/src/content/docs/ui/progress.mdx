...
| `data-color` *(on a parent or the element)* | any imported color scale |
...
- **`surface`** *(default)* — bordered track (`--base-color-a3` + 1px `--base-color-a5` inset), solid `--color-9` fill.
- **`soft`** — flat `--base-color-a4` track, desaturated `--color-a8` fill. Less visual weight.
