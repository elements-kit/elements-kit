---
title: Toolbar
description: CSS-only top and bottom bars with an iOS-style large title that collapses as you scroll — regions by child order, grouped rows, surface, clean and soft variants.
---
...
Without a timeline or `--scroll-y`, the progress properties keep their initial values: a plain bar and a static large title. Keep `--scroll-y` unregistered and without a fallback, such as `var(--scroll-y, 0px)`; a default of 0 would hide the bar's title and background when no script runs.
