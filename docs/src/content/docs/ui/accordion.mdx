...
| `data-color` *(on parent or self)* | any imported color scale |
...
Set `data-color="<color>"` on the accordion or any ancestor to theme the accent. Light/dark flips automatically via the `.dark` class on a parent — see [Light & dark](/ui/styles#light--dark).
