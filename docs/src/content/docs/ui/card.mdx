...
<StorybookEmbed story="ui-card--surface" />
...
- **`surface`** — 1px hairline border (`--neutral-a5`). Default. The everyday card.
- **`elevated`** — same border plus a tailored drop shadow. Uses dual outer-on-element + inner-on-`::after` shadows (`--card-elevated-box-shadow-outer` / `-inner`) with matching 6-layer counts across neutral/hover/active so the lift transitions smoothly. Use for primary, hover-able cards.
- **`borderless`** — no border, no background. Hover fills with `--neutral-a3`. Use inside dense lists or as clickable rows.
...
| `data-inset="fill"` | every side | A scroller filling the card, e.g. a [long dialog](/ui/toolbar#a-long-dialog). Pad its content with `var(--card-padding)` |
