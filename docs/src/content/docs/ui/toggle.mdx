...
| `data-layout` | `stacked` — icon above the label, same geometry as a [stacked button](/ui/button#icon-and-label); `stacked-icon` — no padding, the state on a highlight around the icon (see [Stacked](#stacked)) |
...
Add `data-layout="stacked"` to put the icon above the label. The geometry matches a [stacked button](/ui/button#icon-and-label): at size `2`, a 24px icon, a 2px gap, and a medium-weight caption in `--font-size-0` (10px) on a 16px line, in an item at least 56px square. The line scales with the size, and size `3` captions use `--font-size-1`.
