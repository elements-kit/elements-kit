...
Add `data-layout="stacked"` to put the icon above the label. The geometry matches a [stacked button](/ui/button#icon-and-label): at size `2`, a 24px icon, a 2px gap, and a medium-weight caption in `--font-size-0` (10px) on `--line-height-1`, in an item at least 56px square. Size `3` captions use `--font-size-1` on `--line-height-2`.
