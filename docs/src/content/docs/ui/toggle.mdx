...
The pressed state uses accent (`--accent-*`) tokens, which fall back to neutral when no `data-accent` is set. Every variant changes the fill, the text color, and the weight (`500 → 600`) on press, so pressed stays distinct from rest even when everything is gray.
...
Add `data-layout="stacked"` to put the icon above the label. The geometry matches a [stacked button](/ui/button#icon-and-label): at size `2`, a 20px icon, a 4px gap, and a medium-weight label in `--font-size-1` on `--line-height-1`, in an item at least 56px square. Size `3` labels use `--font-size-2` on `--line-height-2`.
...
A stacked toggle widens for its label. When its container constrains the width, as in a navigation rail, the label wraps into balanced lines and the toggle grows taller. The label keeps one weight when pressed, so a toggle never changes size on press.
