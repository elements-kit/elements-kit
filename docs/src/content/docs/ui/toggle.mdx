...
The rest state uses neutral tokens; the pressed state uses accent (`--accent-*`) tokens that fall back to neutral via [accent/neutral.css](https://github.com/) when no `data-accent` is set. Both variants stack three hue-independent signals — alpha step, ring/weight, and `font-weight: 500 → 600` — so pressed stays clearly distinct from rest even in the all-gray case.

- **`surface`** *(default)* — thin `--neutral-a5` ring + muted `--neutral-a11` text at rest. Pressed: soft `--accent-a5` fill + stronger `--accent-a8` ring + `--accent-12` text. Hover bumps the pressed bg one alpha step deeper (`--accent-a6`).
- **`soft`** — `--neutral-a3` tinted track at rest. Pressed: `--accent-a5` accent-tinted track + `--accent-12` text.
