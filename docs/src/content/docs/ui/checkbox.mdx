...
A native `<input type="checkbox">` styled via class + data attributes from the optional [styles layer](/ui/styles). Two variants, three sizes, color theming, high-contrast and disabled states. The checkmark is an inline SVG mask and the indeterminate dash a drawn bar, both recolored via `currentColor` — no JS.

<StorybookEmbed story="ui-checkbox--surface" />
...
- **`:checked`** — renders a checkmark (SVG mask, sized by the indicator) in the indicator slot.
- **`:indeterminate`** — renders a horizontal bar. The browser doesn't expose this via HTML — set it from JS (`el.indeterminate = true`) and CSS picks it up.
