...
- The wrapped `<input>` / `<textarea>` has its background, border, and outline stripped — the wrapper owns the chrome. A textarea keeps its `--text-input-padding` next to top/bottom affixes; an input drops its text-indent after a leading affix. Corners shared with an affix are squared.
- Affixes get `display: flex; align-items: center; cursor: text` and inline padding of `--space-1` / `--space-2` / `--space-3` by size — override it on the affix to change or remove it.
