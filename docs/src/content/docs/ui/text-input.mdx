...
import TEXT_INPUT from "@/playground/files/ui-text-input.tsx?raw";
...
<Playground
  provider={{ files: { "/main.tsx": { code: TEXT_INPUT, active: true } } }}
  preview={{}}
/>
...
- **`surface`** *(default)* — `--color-surface` background, 1px `--base-color-a7` border, `--focus-8` focus ring.
- **`soft`** — `--color-a3` background, no border, `--color-8` focus ring.
...
- **Disabled / read-only** — `var(--base-color-a11)` text, dimmed background. When the input is empty (`:placeholder-shown`), cursor becomes `var(--cursor-disabled)` and propagates to affixes inside the wrapper.
