...
| `data-color` *(on a parent or the container)* | any imported color scale (`mint`, `blue`, `iris`, …) |
...
- **`surface`** *(default)* — Radix-style: surface track tinted with `--base-color-a3`, indicator fills with `--color-background` and a 1px hairline + 1px shadow. Inactive text uses `--base-color-a11`; active jumps to `--base-color-12` and `--font-weight-medium`.
- **`soft`** — accent-tinted track using `--color-a3`. Inactive text uses `--color-a11`; active uses `--color-12`. Pairs well with `data-color`.
...
Set `data-color="<color>"` on the container or any ancestor (e.g. `<body>`) to theme the accent. Affects the `soft` variant's track and the focus ring. Light/dark flips automatically via the `.dark` class on a parent — see [Light & dark](/ui/styles#light--dark).
...
7. **Don't rely on color alone.** Active segment also bumps `font-weight: var(--font-weight-medium)` and `letter-spacing: var(--tab-active-letter-spacing)` — Radix-mirrored.
