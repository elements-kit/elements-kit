...
Add `data-layout="stacked"` on the container to put each icon above its label, as in a tab bar. Segments follow iOS control-group item metrics. At size `2` a segment is 56px tall, with a 22px icon (overridable with `--seg-stacked-icon-size`), a 5px gap, and a 12px medium-weight label on an 18px line that truncates. Every length scales with the size: 42, 56 and 70px tall for sizes `1`–`3`. The weight doesn't change on selection, so segments never reflow.
