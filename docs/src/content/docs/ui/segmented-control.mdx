...
<StorybookEmbed story="ui-segmented-control--surface" />
...
| `data-layout` | `stacked` — icon above the label (see [Icons](#icons)) |
| `data-separators` | `none` — no hairlines between segments (stacked controls never have them) |
...
Put an icon before the label inside each `<label>`; it sits at the start of the segment. Size it in `em` so it follows the control's font size.

```html
<label>
  <input type="radio" name="view" value="list" checked />
  <svg width="1.25em" height="1.25em" aria-hidden="true">…</svg>
  <span>List</span>
</label>
```

For icon-only segments, drop the `<span>` and name each radio with `aria-label`:

```html
<label>
  <input type="radio" name="view" value="grid" aria-label="Grid" />
  <svg width="1.25em" height="1.25em" aria-hidden="true">…</svg>
</label>
```

Add `data-layout="stacked"` on the container to put each icon above its label, as in a tab bar. At size `2` the control is 58px tall: each segment's highlight sits 2px inside the track and holds a 24px icon, a 1px gap, and a medium-weight caption in `--font-size-0` (10px) on a 16px line. The track, the icon and the caption's line scale with the size — the track is 43.5, 58 and 72.5px tall for sizes `1`–`3` — and size `3` captions use `--font-size-1`.

A stacked control fits its container: segments stay equal width, long labels truncate, and there are no separators. The weight doesn't change on selection, so segments never reflow.
