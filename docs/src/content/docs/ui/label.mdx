...
A label styled like Radix Themes' forms: medium-weight `--neutral-12` above a text input, regular text beside a checkbox, radio or switch, or a group heading in a [menu](/ui/menu).
...
| `group` | a heading for a group of options or menu items | regular | `--neutral-a10` | none |
...
Wrapped around a checkbox, radio or switch, the label becomes the option text, as in Radix's checkbox group: regular weight, a `0.5em` gap, and only as wide as its content, so the empty space beside it isn't clickable. The control is centered on the first line's capitals, however many lines the text wraps to. Stack options 4px apart.
...
In an [`x-menu`](/ui/menu), use `data-variant="group"` to head a group of items. The menu adds the item's height and side padding, as in Radix Themes.
