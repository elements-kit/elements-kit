...
A 1px line that separates content, modelled on Radix Themes' separator. It's a native `<hr>`, so it already has the `separator` role.
...
Radix defaults to size `1`. Here the default is the full length, what an `<hr>` usually means.
