...
| `data-layout` | `stacked` — icon above the label; the button grows to fit (see [Icon and label](#icon-and-label)) |
...
Add `data-layout="stacked"` to put the icon above the label, as in iOS control groups. At size `2` it matches the iOS item exactly: 56px tall, a 22px icon, a 5px gap, and a 12px medium-weight label on an 18px line, with 4px inline padding. Every length scales with the size. The label truncates, and `--button-stacked-icon-size` overrides the icon.
...
`text` stacked buttons keep their fit-to-content height.
