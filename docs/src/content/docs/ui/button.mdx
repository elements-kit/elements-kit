...
| `data-layout` | `stacked` — icon above the label; never taller than wide (see [Icon and label](#icon-and-label)) |
...
The height is fixed and `aspect-ratio: 1` keeps the button at least as wide as it is tall: a short label gives a square, a longer one widens it. The label truncates when the button is constrained.

| `data-size` | stacked height | icon | label |
| --- | --- | --- | --- |
| `1` | 42px | 15px | 11px |
| `2` | 56px | 20px | 12px |
| `3` | 70px | 25px | 13px |
...
`text` stacked buttons keep the kit's text padding (8px × 4px at size `2`), so they are 51px tall and still bleed like other text buttons.
