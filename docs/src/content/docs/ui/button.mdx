...
| `data-layout` | `stacked` — icon above the label; at least square, the label wraps when constrained; `stacked-icon` — the same without padding, the state on a highlight around the icon (see [Icon and label](#icon-and-label)) |
...
For a selectable item (a navigation rail or bar), use a [`stacked-icon` toggle](/ui/toggle#icon-indicator).
