...
Add `data-layout="stacked"` to put the icon above the label, as in iOS action items. At size `2` the button is 56px tall, with a 20px icon, a 4px gap, and a medium-weight label in `--font-size-1` on `--line-height-1` (12px on 16px), with at least 4px padding above and below and 8px on the sides. The box and icon scale with the size; the label steps up the type scale at size `3`.
...
| --- | --- | --- | --- |
| `1` | 42px | 15px | `--font-size-1` / `--line-height-1` |
| `2` | 56px | 20px | `--font-size-1` / `--line-height-1` |
| `3` | 70px | 25px | `--font-size-2` / `--line-height-2` |
| `4` | 84px | 30px | `--font-size-2` / `--line-height-2` |
