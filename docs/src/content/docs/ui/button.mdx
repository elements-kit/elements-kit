...
Add `data-layout="stacked"` to put the icon above the label, as in action items. At size `2` the button is 56px tall, with a 24px icon, a 2px gap, and a medium-weight caption in `--font-size-0` on `--line-height-1` (10px on 16px), with at least 4px padding above and below and 8px on the sides. The box and icon scale with the size; the caption steps up to `--font-size-1` at size `3`. The icon carries the meaning, so keep captions short.
...
| --- | --- | --- | --- |
| `1` | 42px | 18px | `--font-size-0` / `--line-height-1` |
| `2` | 56px | 24px | `--font-size-0` / `--line-height-1` |
| `3` | 70px | 30px | `--font-size-1` / `--line-height-2` |
| `4` | 84px | 36px | `--font-size-1` / `--line-height-2` |
