...
<CardGrid>
  <Card title="Compose, don't configure" icon="puzzle">
    Small focused APIs — `signal`, `computed`, `on`, `fromEvent`, `async`. One toolkit covers state, server-state, browser hooks, and UI.
  </Card>
  ...
  <Card title="Designed for the AI age" icon="rocket">
    Primitives compose into higher-level blocks. Swap one block at a time instead of maintaining an overloaded interface.
  </Card>
</CardGrid>

### Compose, don't configure

ElementsKit gives you primitives you combine, not a system you configure. No config object, no plugin tree, no convention to memorize — the call site is the contract. Need a query layer? Compose `async` + `retry` + storage. Need browser-API hooks? Import the utility. The surface grows by adding primitives, not by stacking flags on an existing one.
...
A query layer is `async` + `retry` + storage. Swap the fetcher; leave the retry and storage alone. Swap the store; leave the query alone.

The alternative is one overloaded component tuned through every combination of its configuration. Each new option on that component accumulates breaking changes and deprecations that every consumer has to track and migrate through. Primitives avoid that maintenance tax by keeping each unit small enough to replace rather than extend.
