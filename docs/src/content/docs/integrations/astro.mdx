...
Editing an island component swaps it in place — no page reload. The integration installs a dev-only Vite plugin that gives each component module an accept boundary; the edited island is disposed and re-mounted with the new code.

The island **re-mounts**, so its state resets. Anything that must survive an edit goes in a separate module, which the edit doesn't invalidate:
...
Edits that reach beyond the island — the parent `.astro` file, `astro.config.mjs` — still reload the page, as does a component module that renders no JSX (nothing marks it as an island). Production builds carry no HMR code.
