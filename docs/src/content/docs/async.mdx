...
`Async` implements `Symbol.dispose`, so you can use the `using` keyword to stop it automatically when it goes out of scope:
...
An `Async` instance is also callable as a signal. Calling `op()` returns `op.result` and tracks it as a reactive dependency:
...
Use `run()` for one-shot operations (e.g. mutations). `await op.run(args)` resolves to the fulfilled value:
...
const result = await mutation.run(42);
```
...
const query = async(() =>
  fetch("/api/todos")
    .then((r) => r.json())
    .then(cache), // value flows through and is saved to storage
).start();
```

## Reactive Reruns with `start()`
...
Compose with `retry` from `elements-kit/utilities/retry` to automatically retry on failure. Pass an optional delay for backoff between attempts:
...
const fetchTodo = async((id: number) =>
  ...
  }, 3, (n) => n * 500)(), // 0ms, 500ms, 1000ms between failures
).start();
```

## Refetch on Network Reconnect

Read `online()` inside the async function. When connectivity is lost and restored, `start()` re-runs automatically:
...
const data = signal()
const query = async(() => {
  if (!online()) return untracked(data); // returns stale value while offline
...
Read `windowFocused()` inside the async function to re-execute whenever the user returns to the tab:
...
const query = async(() => {
  windowFocused(); // re-runs when window regains focus
...
Read `timer.timestamp()` inside the async function to re-execute on every tick:
...
const fetchTodo = async(() => {
  timer.timestamp(); // re-runs every 10 seconds
